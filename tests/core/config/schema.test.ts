...
describe('providerConfigSchema', () => {
  ...
  it('accepts openai config with required fields', () => {
    const result = providerConfigSchema.safeParse({
      openai: { apiKey: 'sk-test-key-12345678' },
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.openai?.baseUrl).toBe('https://api.openai.com');
    }
  });

  it('rejects openai without apiKey', () => {
    const result = providerConfigSchema.safeParse({
      openai: {},
    });
    expect(result.success).toBe(false);
