...
describe('getSupportedProviders', () => {
  it('returns anthropic, openai, and ollama', () => {
...
describe('createProvider', () => {
  ...
  it('throws for unknown provider name', () => {
    const config: ProviderConfig = {};
    expect(() => createProvider('unknown-provider' as any, config)).toThrow(ProviderError);
  });

  it('creates an OpenAI provider with valid config', () => {
    const config: ProviderConfig = {
      openai: {
  ...
  it('throws ProviderError when OpenAI config is missing', () => {
    const config: ProviderConfig = {};

    expect(() => createProvider('openai', config)).toThrow(ProviderError);
  });
});
