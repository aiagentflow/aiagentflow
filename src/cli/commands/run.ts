...
export const runCommand = new Command('run')
    ...
    .action(async (task: string, options: { auto?: boolean; batch?: boolean; mode?: string; stopOnFailure?: boolean; context?: string[]; stream: boolean; dryRun?: boolean; isolate?: boolean; reviewPlan?: boolean; approvalGates?: string[]; parallel?: number; maxTokens?: number; maxCost?: number; summary: boolean; pr?: number; issue?: number }) => {
        ...
        try {
            ...
            // --pr: fetch PR context and build task from review comments
            let resolvedTask = task;
            let prContext;
            if (options.pr) {
                try {
                    logger.info(`Fetching PR #${options.pr}...`);
                    prContext = await fetchPR(options.pr);
                    resolvedTask = buildPRTask(prContext);
                    logger.info(`PR #${options.pr}: "${prContext.title}"`);
                } catch (err) {
                    logger.error(`Failed to fetch PR: ${err instanceof Error ? err.message : String(err)}`);
                    process.exit(1);
                }
            }

            // --issue: fetch issue context and build task
            if (options.issue) {
                try {
                    ...
                    logger.error(`Failed to fetch issue: ${err instanceof Error ? err.message : String(err)}`);
                    process.exit(1);
                }
            }
