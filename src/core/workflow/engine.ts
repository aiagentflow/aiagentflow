 ...
 */
export function transition(context: WorkflowContext, event: WorkflowEvent): WorkflowContext {
  ...
  const isFixLoop =
    nextState === 'review_rejected' ||
    (nextState === 'fix_applied' && currentState === 'tests_failed');
...
function resolveNextState(current: WorkflowStateValue, event: WorkflowEvent): WorkflowStateValue {
  switch (event.type) {
    case 'SPEC_READY':
      return WorkflowState.SpecCreated;
    case 'PLAN_APPROVED':
      return WorkflowState.PlanApproved;
    case 'CODE_GENERATED':
      return WorkflowState.CodeGenerated;
    case 'REVIEW_DONE':
      return event.payload.approved ? WorkflowState.ReviewDone : WorkflowState.ReviewRejected;
    case 'TESTS_WRITTEN':
      return WorkflowState.TestsWritten;
    case 'TESTS_PASSED':
      return WorkflowState.TestsPassed;
    case 'TESTS_FAILED':
      return WorkflowState.TestsFailed;
    case 'FIX_APPLIED':
      return WorkflowState.FixApplied;
    case 'QA_APPROVED':
      return WorkflowState.QAApproved;
    case 'QA_REJECTED':
      return WorkflowState.CodeGenerated;
    case 'ABORT':
