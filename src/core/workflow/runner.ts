  ...
  switch (role) {
    ...
    case 'reviewer': {
      ...
      if (evaluation.totalIssues > 0) {
        logger.info(
          `QA policy: ${evaluation.criticalCount} critical, ${evaluation.warningCount} warning(s), ${evaluation.totalIssues - evaluation.criticalCount - evaluation.warningCount} nit(s)`,
        );
      }
    ...
    case 'judge': {
      const passed =
        content.toUpperCase().includes('PASS') && !content.toUpperCase().includes('FAIL');
      if (passed) {
